# ADR 003: Field-Level Authorization for P90 Solar Cost Data
...
P90 solar cost data (the 90th percentile $/W for a system-size/panel-tier/location/installer-type combo) is commercially sensitive. In a real product, it would be gated behind a paid tier or require account verification. The API must return partial data for unauthenticated users — showing p25/p50/p75 — while hiding p90.

## Decision

Implement **field-level authorization** in the `CostBandType` resolver: the `p90` field returns `null` when `info.context.request.user.is_authenticated` is `False`.
...
A query-level permission check (`@strawberry.permission.BasePermission`) would reject the entire `costBands` query for unauthenticated users. This is too coarse — anonymous users should be able to browse the table and see most data. Field-level control allows a degraded but useful experience without authentication.
...
- `CostBandType.p90` is `Optional[float]` in the schema and in TypeScript types.
